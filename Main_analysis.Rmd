...
#try correlations with mixed data type
AllMatrix <- model.matrix(~.-1, data=train) #This turns all categorical variables into dummies
corAll <- cor(AllMatrix, use = "pairwise.complete.obs")
corAll_sort <- as.matrix(sort(corAll[,'SalePrice'], decreasing = TRUE))
...
testdf <- data.frame(x1 = as.factor(c("a","a","b","c","d")), x2 = c(1,2,3,4,5),x3 = as.factor(c('Y','Y','N','N', 'X')))
